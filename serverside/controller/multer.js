...
exports.resizePhoto=async(req,res,next)=>{
  ...
  req.file.filename= `${req.body.email}-user.jpeg`;
    sharp(req.file.buffer).resize(500,500).toFormat('jpeg').jpeg({quality:80}).toFile(`my-app/build/media/static/${req.file.filename}`)
    next()
}
exports.resizePhoto2=async(req,res,next)=>{
  ...
  req.file.filename= `${Date.now()}-post.jpeg`;
    await sharp(req.file.buffer).resize(500,500).toFormat('jpeg').jpeg({quality:80}).toFile(`my-app/build/static/media/${req.file.filename}`)
    next()
}